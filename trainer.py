...
class Trainer(object):
    ...
    def initialize_optimizer_and_scheduler(self):
        self.optimizer = torch.optim.SGD(
            self.model.get_config_optim(self.args.lr, self.args.lrp),
            lr=self.args.lr,
    ...
    def on_end_epoch(self, is_train=False):
        ...
        if is_train:
            ...
            print(
                "OP: {OP:.3f}\t OR: {OR:.3f}\t OF1: {OF1:.3f}\t"
                "CP: {CP:.3f}\t CR: {CR:.3f}\t CF1: {CF1:.3f}".format(OP=OP, OR=OR, OF1=OF1, CP=CP, CR=CR, CF1=CF1)
            )
            print(
                "OP_3: {OP:.3f}\t OR_3: {OR:.3f}\t OF1_3: {OF1:.3f}\t"
    ...
    def run_iteration(self, data_loader, is_train=True):
        ...
        for i, data in enumerate(data_loader):
            ...
            if is_train and i % self.args.display_interval == 0:
                print(
                    "{}, {} Epoch, {} Iter, Loss: {:.4f}, Data time: {:.4f}, Batch time: {:.4f}".format(
    ...
    def save_result(self, model_dir, is_best=False):
        ...
        with open(res_path, "w") as fid:
            for i in range(self.meters["ap_meter"].scores.shape[0]):
                fid.write(
                    "{},{},{}\n".format(
                        self.meters["ap_meter"].filenames[i],
