...
class COCO2014(Dataset):
    def __init__(self, root, transform=None, phase="train", filter_labels=None):
        ...
        print(
            "[dataset] COCO2014 classification phase={} number of classes={}  number of images={}".format(
                phase, self.num_classes, len(self.img_list)
            )
        )
    ...
    def get_anno(self, filter_labels: set):
        ...
        if filter_labels is not None:
            ...
            self.img_list = [
                {"file_name": x["file_name"], "labels": list(set(x["labels"]).intersection(filter_labels))}
